...
def harrison_ford
  ...
  .joins(castings: :actor) #use this format when there's no listed through association. If there were a through association, just list that association name
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group(:id, :title).order('count(actor_id) DESC').limit(3)
